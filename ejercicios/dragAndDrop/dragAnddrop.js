...
function dragLeave(evento) {
  ...
  Permite que el evento ocurra sólo en un elemento, el del evento 
  Evita que un mismo evento se repita para todos los elementos contenedores
  Por ejemplo, un li está dentro de un ul que está dentro de un div. Si hago click en el li, ese click afecta tambien al ul y al div.
  con stopPropagation solo afecta el li y deja de "propagarse" al resto de los elementos contenedores.
  Más info: https://developer.mozilla.org/en-US/docs/Web/API/Event/stopPropagation
  DE MDN: Nota: Este evento no detiene ningún comportamiento por "default", (por ejemplo hacer click en links)
  Si además se desea detener los comportamientos por default (por ejemplo, Un drag and drop de un link,pero sin
  que ocurra el click del link), se debe usar el evento preventDefault().
  */
